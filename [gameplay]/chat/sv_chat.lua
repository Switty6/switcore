...
-- ============================================================
-- CANALE IMPLICITE SWITCORE
-- Inregistrate direct in tabela modes; vor fi trimise clientilor
-- via evenimentul chat:init la fiecare conectare.
-- ============================================================
local function addDefaultMode(modeData)
    modeData.resource = GetCurrentResourceName()
    modes[modeData.name] = modeData
end

-- Local: broadcast implicit (pentru proximity real inlocuieste cb-ul cu logica de distanta)
addDefaultMode({
    name        = 'local',
    displayName = 'Local',
    color       = '#e8f4ff',
    isChannel   = true,
    cb          = function(source, message, hook) end
})

-- Global: trimis tuturor jucatorilor, apare in toate canalele (isGlobal)
addDefaultMode({
    name        = 'global',
    displayName = 'Global',
    color       = '#00b4ff',
...
-- OOC: Out Of Character, vizibil tuturor
addDefaultMode({
    name        = 'ooc',
    displayName = 'OOC',
    color       = '#ffd166',
    isChannel   = true,
    cb          = function(source, message, hook)
        hook.updateMessage({ template = '^3(( <b>{0}</b>: {1} ))^0' })
        hook.setRouting(-1)
    end
})

-- Admin: doar jucatorii cu ACE permission "chat.mode.admin"
addDefaultMode({
    name        = 'admin',
    displayName = 'Admin',
    color       = '#ff4d6a',
    seObject    = 'chat.mode.admin',
    isChannel   = true,
    cb          = function(source, message, hook)
        hook.updateMessage({ template = '^1[ADMIN]^0 <b>{0}</b>: {1}' })
        hook.setSeObject('chat.mode.admin')
    end
})

-- Faction: placeholder - ruteaza catre toti pana la integrarea sistemului de factiuni
addDefaultMode({
    name        = 'faction',
    displayName = 'Faction',
    color       = '#00c97a',
    isChannel   = true,
    cb          = function(source, message, hook)
        hook.updateMessage({ template = '^2[FACTION]^0 <b>{0}</b>: {1}' })
        hook.setRouting(-1)
    end
})
