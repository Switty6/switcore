...
export default Vue.component('message', {
  ...
  methods: {
    ...
    parseEmoji(str: string): string {
      const map: Record<string, string> = {
        ':smile:':     '😊', ':grin:':      '😁', ':laugh:':    '😂', ':lol:':      '😂',
        ':sad:':       '😢', ':cry:':       '😭', ':angry:':    '😠', ':mad:':      '😡',
        ':thumbsup:':  '👍', ':thumbsdown:':'👎', ':ok:':       '👌', ':clap:':     '👏',
        ':heart:':     '❤️', ':fire:':      '🔥', ':check:':    '✅', ':x:':        '❌',
        ':warning:':   '⚠️', ':money:':    '💰', ':gun:':      '🔫', ':police:':   '👮',
        ':car:':       '🚗', ':house:':     '🏠', ':phone:':    '📱', ':star:':     '⭐',
        ':skull:':     '💀', ':wave:':      '👋', ':eyes:':     '👀', ':pray:':     '🙏',
        ':shrug:':     '🤷', ':facepalm:':  '🤦', ':crown:':    '👑', ':muscle:':   '💪',
      };
      return str.replace(/:([a-z0-9_]+):/g, (match, code) => map[`:${code}:`] || match);
    },
    colorize(str: string): string {
      let s = "<span>" + colorTrans(this.parseEmoji(str)) + "</span>";
