...
local function concatArgs(args, from)
    local r = table.concat(args, ' ', from)
    return r ~= '' and r or nil
end

function CommandLogger.logCommand(source, commandName, args, metadata)
    if not (Config and Config.LOG_COMMANDS ~= false) then return false end
...
CommandLogger.registerCommand('unban', function(source, args, rawCommand)
    ...
    local target = args[1]
    local reason = concatArgs(args, 2)
